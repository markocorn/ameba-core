...
public class Max extends Node {
    ...
    public Max(int minInpCollectors, int maxInpCollectors) throws Exception {
    ...
    public void clcNode() {
        ...
        for (CollectorTarget collector : getCollectorsTargetConnected()) {
            if (collector.getSignal() > max) max = collector.getSignal();
        }
        if (getCollectorsTargetConnected().isEmpty()) max = 0.0;
        getCollectorsSource().get(0).setSignal(max);
    }
}
