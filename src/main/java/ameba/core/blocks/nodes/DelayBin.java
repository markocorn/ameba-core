...
public class DelayBin extends NodeMem {
    ...
    public DelayBin(Signal initial, int par, int[] parLimits) throws Exception {
    ...
    public void clcNode() throws Exception {
        buffer.add(getInpCollectorsBin().get(0).getSignal());
        buffer.remove(buffer.get(0));
        getOutCollectorsBin().get(0).setSignal(buffer.get(0));
    }
