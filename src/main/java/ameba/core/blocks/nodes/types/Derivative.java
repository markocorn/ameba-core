 ...
 * @author Marko
 */
public class Derivative extends NodeMem {

    //Initial value
    private double initValue;
    private double signalOld;

    public Derivative(double initial, Double par, Double[] parLimits) throws Exception {
    ...
    @Override
    public void clcNode() {
        if (getParamsDec().get(0) == 0.0) {
            getCollectorsSource().get(0).setSignal(0.0);
        } else
            getCollectorsSource().get(0).setSignal((getCollectorsTarget().get(0).getSignal() - signalOld) / getParamsDec().get(0));
        signalOld = getCollectorsTarget().get(0).getSignal();
    }
