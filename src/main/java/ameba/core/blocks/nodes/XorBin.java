...
public class XorBin extends Node {
    ...
    public void clcNode() throws Exception {
        ...
        for (int i = 1; i < getInpCollectors().size(); i++) {
            getOutCollectorsDec().get(0).getSignal().setValueBoolean(getInpCollectorsConnected(Boolean.class).get(i - 1).getSignal().getValueBoolean() ^ getInpCollectorsConnected(Boolean.class).get(i).getSignal().getValueBoolean());
        }
    }
}
